...
function sanitizeRawOffer(offer) {
  const sanitized = Object.assign({}, offer);
  if ((sanitized.items_to_give || []).length > 0) {
    sanitized.items_to_give = sanitized.items_to_give.filter(
      item => !itemMalformed(item),
    );
  }
  if ((sanitized.items_to_receive || []).length > 0) {
    sanitized.items_to_receive = sanitized.items_to_receive.filter(
      item => !itemMalformed(item),
    );
  }
  return sanitized;
